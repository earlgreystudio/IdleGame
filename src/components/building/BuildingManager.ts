...
export class BuildingManager {
  ...
  demolishBuilding(buildingId: string): BuildingActionResult {
    ...
    Object.entries(cost).forEach(([resource, amount]) => {
      if (amount) {
        this.centralStateManager.addResource(resource, Math.floor((amount as number) * 0.5), 'Building demolition refund');
      }
    });
  ...
  private canAffordCost(cost: Record<string, number>): boolean {
    return Object.entries(cost).every(([resource, amount]) => {
      return amount ? this.centralStateManager.hasResource(resource, amount) : true;
    });
  }
  ...
  private consumeResources(cost: Record<string, number>): void {
    Object.entries(cost).forEach(([resource, amount]) => {
      if (amount) {
        this.centralStateManager.removeResource(resource, amount, 'Building construction cost');
      }
    });
  }
